...
> -   **视频过滤支持「首页、播放页、搜索页、热门页、频道页」**
> -   「通用项」对全站生效
> -   按下图打开菜单，在每个页面进行自定义净化，实时生效
...
-   目前支持「首页、播放页、影视番剧播放页、直播间、搜索页、动态页、热门/排行榜页、频道页」，各个页面均有独立菜单
-   混搭使用各种功能，调节浏览器缩放比例，可调节出满意的页面效果
-   默认用户已登录，默认开启少数基础屏蔽功能，大会员体验最佳
...
> [!CAUTION]
>
> -   **如果屏蔽了大量内容，请定时备份插件数据，更换电脑或浏览器务必备份，[备份方法](#数据导入与导出)**

-   视频过滤支持「UP主、BV号、标题关键词、视频时长」四种模式
-   页面支持「首页、播放页、搜索页、频道页、热门页（热门/每周必看/排行榜）」
-   **每个页面有独立的功能开关，但黑名单、白名单全站共用一份**
-   **白名单权限高于黑名单，命中白名单的视频不会被隐藏**
-   「标题关键词过滤」和「标题关键词白名单」关键词大小写不敏感。均支持正则（大小写不敏感），正则需使用 `/ ... /` 括起来，如：`/abc|\d+/`
...
> [!CAUTION]
>
> -   **如果你在意推送质量，遇到不确定/不喜欢的 UP 主时，不要点开视频，不要点开个人页!!!**
> -   点开页面后，推荐系统会误认为你喜欢，导致推送变差，请复制链接并使用无痕浏览
