...
`));
	...
	var convertDateToDays = (dateStr) => {
		...
		if (/小时|小時|分钟|分鐘/.test(dateStr)) return 0;
		const daysAgoMatch = dateStr.match(/^(\d+)天前$/);
		if (daysAgoMatch) return Number(daysAgoMatch[1]);
		const now = new Date();
		...
		if (yesterdayMatch) {
			...
			return (today - target.getTime()) / 864e5;
		}
		...
		if (monthDayMatch) {
			...
			if (target > startOfToday) target = new Date(now.getFullYear() - 1, month - 1, day).getTime();
			return (startOfToday - target) / 864e5;
		}
		...
		if (/^\d{4}-\d{1,2}-\d{1,2}$/.test(dateStr)) {
			...
			return (startOfToday - new Date(year, month - 1, day).getTime()) / 864e5;
		}
		return 0;
	};
